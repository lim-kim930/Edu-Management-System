...
export default {
  ...
  methods: {
    ...
    createSeal(company, ecompany, name, blockheight) {
      ...
      const ecount = ecompany.length;// 字数
      const eangle = (4.9 * Math.PI) / (3 * (ecount));// 字间角度   
      const echars = ecompany.split("");
      let ec;
      ...
      const ecount1 = blockheight.length;// 字数   
      const eangle1 = (1.17 * Math.PI) / (4 * (ecount1));// 字间角度   
      const echars1 = blockheight.split("");
      let ec1;
      for (let i = 0; i < ecount1; i++) {
        ec1 = echars1[i];// 需要绘制的字符
        if (i == 0)
