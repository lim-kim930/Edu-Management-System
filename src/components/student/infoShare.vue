  ...
  <div>
    ...
    >
      ...
      >
        ...
        <el-form-item class="md" label="个性化信息" required>
          <h4 v-show="!selfData[0].md">暂无信息</h4>
          <el-checkbox v-model="ruleForm.selfChecked">选择</el-checkbox>
          <mavonEditor
            :toolbars="toolbars"
            :autofocus="false"
            defaultOpen="preview"
        ...
        <el-form-item label="目的企业和岗位" required>
          <el-cascader
            ...
            clearable
          ></el-cascader>
          <span v-show="target_show">{{ target }}</span>
        </el-form-item>
        <el-form-item
          label="有效时间"
          prop="date"
    ...
    <el-dialog :visible.sync="previewVisible">
      ...
      <el-button
          ...
          >{{this.ruleForm.selfChecked?(this.switch === 1?"查看个性化预览":"返回"):"未选择个性化信息"}}</el-button>
      <el-button
            type="primary"
            @click="confirm()"
            ...
            if(JSON.parse(sessionStorage.getItem("com")).Name === "") {
              for(let i = 0; i < this.options.length; i++)
                if(this.options[i].value === JSON.parse(sessionStorage.getItem("com")).CompanyCode)
                  this.target = " PS: 正在回应" + this.options[i].label + "的简历请求, 请选择对应企业的岗位"
            }
            else{
              this.target = "已为您选择: " + JSON.parse(sessionStorage.getItem("com")).Name + "/" + JSON.parse(sessionStorage.getItem("com")).job
