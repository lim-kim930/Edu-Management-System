<template>
  <el-form ref="form" class="form1" v-loading="loading" element-loading-text="拼命加载中">
    ...
    <h4 v-show="JSON.stringify(oldData) !== '{}'">上次确认的信息</h4>
    ...
    <h4 v-show="JSON.stringify(oldData) !== '{}'">新的信息</h4>
  ...
  methods: {
    ...
    getFileInfo() {
      ...
      this.axios({
        ...
        if (response.data.data.Body.data_map.rank) {
          ...
          this.getInfo(rank[id].MajorName, rank[id].GPA, rank[id].Rank)
        }
        else
          this.getInfo()
      }).catch(() => {
    ...
    getInfo(MajorName, GPA, Rank) {
      ...
      this.axios({
        ...
        if (MajorName)
          if (GPA !== value.GPA.Value || Rank !== value.Rank.Value || MajorName !== value.MajorName.Value) {
            this.oldData = {
              MajorName: { Value: MajorName, VisibleName: "专业名称" },
              GPA: { Value: GPA, VisibleName: "GPA" },
              Rank: { Value: Rank, VisibleName: "排名" },
            }
